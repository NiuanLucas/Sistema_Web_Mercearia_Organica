...
?>
    ...
    <legend>Produtos disponíveis para a sua área de entrega</legend>
	<?php
	   ...
	   else
	   {
        ...
        {			
			?>
		    <legend>Pedidos/ofertas anteriores</legend>
            <table class="table table-bordered">
                <thead>
                    <tr>
                        <th>#</th>
			...
			{				
				  ...
				  <tr class="<?php echo( $row['ped_fechado']==1 ? "success": ($row['ped_id'] ? "info" : "")); ?>" >
					 ...
					 <td><?php echo($row['cha_dt_entrega']); ?></td>
					 <td><?php echo($row['cha_dt_max']); ?> </td>  
					 <td class="hidden"> <?php echo("<a class=\"btn btn-default btn-sm\" href=\"chamada_info.php?action=" . ACAO_EXIBIR_LEITURA .  "&amp;cha_id=" . $row['cha_id'] . "\"><i class=\"glyphicon glyphicon-leaf\"></i> ver chamada</a>");?></td>
