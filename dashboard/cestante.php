...
<?php
		...
		{			
			 ...
			 if($res && mysqli_num_rows($res))
			 {
				...
				adiciona_mensagem_status(MSG_TIPO_ERRO,"Já existe cestante cadastrado com o email " . request_get('usr_email',""). ". Favor informar outro e tantar salvar novamente; ou então solicitar ao responsável a utilização da respectiva conta, hoje em nome de '$usr_outro'.");		
				$salvar=false;		
			 }	
			 ...
			 if($res && mysqli_num_rows($res))			 
			 {
				 $row = mysqli_fetch_array($res,MYSQLI_ASSOC);
			 	$usr_outro = $row["usr_nome_completo"];
				adiciona_mensagem_status(MSG_TIPO_ERRO,"Já existe cestante cadastrado com o nome curto " . request_get('usr_nome_curto',""). ". Favor informar outro e tantar salvar novamente; ou então solicitar ao responsável a utilização do respectivo nome, hoje associado a '$usr_outro'.");				
				$salvar = false;
			 }
...
<table class="table-condensed table-info-cadastro" style="width: inherit !important;">
		<tbody>
    		...
    		<tr>
				<th>Aréa de Entrega:</th> <td><?php echo($nuc_nome_curto); ?></td>
			</tr> 
    		<tr>
			...
			<?php 
             ...
             ?>
                     ...
                     <?php
                                ...
                                if($res3 && mysqli_num_rows($res3))
                                {
                                  ...
                                  <strong>Para pedido com chamada ainda em aberto:</strong><br>
                                   &nbsp; - Se alterar a Aréa de Entrega  do respectivo pedido, o mesmo será mudado támbem. Como consequência: caso o pedido contenha produtos de produtores que não atendem esta nova Aréa de Entrega, estes produtos serão automaticamente removidos do pedido. <br>
                                 ...
                                 Para estes casos o pedido não pode mais ser alterado, pois se o prazo para edição terminou significa que a logística já começou. Então o pedido fica congelado. Se o cestante mudar de Aréa de Entrega, precisará resgatar este pedido na Aréa de Entrega antiga, onde será entregue. Se o cestante ficar inativo, precisará resolver o resgate / pagamento deste pedido. <br><br>                                 
                    ...
                    ?>   
                     ...
                     <div class="form-group">
                      <label class="control-label col-sm-2" for="usr_nuc">Aréa de Entrega</label>
                      <div class="col-sm-3">                                       
