...
<table class="table-condensed table-info-cadastro">
        ...
        <div class="panel-footer">
         	...
         	&nbsp;&nbsp;
         	<a class="btn btn-danger" href="produtor.php?action=<?php echo(ACAO_EXIBIR_LEITURA); ?>&forn_id=<?php echo($forn_id); ?>&excluir=1"><i class="glyphicon glyphicon-remove"></i> apagar produtor</a>&nbsp;&nbsp;
...
<?php 
  ...
  // REMOVE TABLE
        ...
        $excluir = "DELETE FROM fornecedores ";
        $excluir .= "WHERE  forn_id = {$fID} ";
        $operacao_excluir = mysqli_query($conecta, $excluir);
          if(!$operacao_excluir){
            die("Erro na Exclusão do Banco de Dados </br></br></br> " . $excluir . " </br></br></br> ");
          } else  {
            echo " <script>window.location.href = 'produtores.php';</script>";
                }
