...
//					$sql.= "AND prod_ini_validade <= NOW() AND prod_fim_validade >= NOW() ";
				...
				?>				 
				  <tr <?php echo($classe_arquivado); ?>>
					 ...
					 <td>
					 <a href="produtor.php?action=0&amp;forn_id=<?php echo($row['forn_id']);?>"><?php echo($icone_arquivado);?><?php echo($row['forn_nome_completo']);?> </td> 
                     <td><?php echo($row['forn_nome_curto']);?></td> 
					 <td><?php echo($row['forn_email']);?> </td>                     
					 <td>&nbsp;<?php echo($row['forn_qtde_produtos']);?> &nbsp; </br> <a class="btn btn-default btn-xs" href="produtos.php?prod_forn=<?php echo($row['forn_id']);?>"><i class="glyphicon glyphicon-search"></i> consultar</a></td>                     
				  </tr>
				<?php 
				     }
				   }
