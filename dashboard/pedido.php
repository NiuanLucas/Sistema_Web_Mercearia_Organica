...
<?php
		...
		{
			$sql = "SELECT (cha_dt_max<now()) somente_leitura, usr_nome_curto,usr_endereco, ped_usr, ped_usr_associado, usr_nome_completo, usr_contatos, prodt_nome, ";
...
?>
	...
	<legend>
	<!-- Pedido de <?php echo(($prodt_nome) . " - " . ($cha_dt_entrega)); ?> -->
	Pedido: <?php echo(($cha_dt_entrega)); ?> 
	</legend>

	<?php 
	$nome_usuario = $usr_nome_completo;
	$end_usuario = $usr_endereco;
	?>
      <div class="row">
       	<div class="col-md-5"><strong>Cliente</strong>: <?php echo($usr_nome_curto);?> (<?php echo($usr_contatos ? $usr_contatos : "sem contato informado"); ?>) </div>
        <div class="col-md-4"><strong>Área de Entrega: </strong>    <?php echo($nuc_nome_curto);?></div>
     	<div class="col-md-3 hidden-print hidden"><strong>Associado:</strong> <?php echo($ped_usr_associado==1 ? "Sim" : "Não")?></div>
		...
		$texto_indicador_categoria_preco = "<span class='label label-info'>para o seu caso, vale este </span>&nbsp;";
		  ...
		  ?>  
           ...
           <tr>
             ...
             <td>
             	R$ <?php echo(formata_moeda($cha_taxa_percentual)); ?>
             <!-- <?php echo(formata_moeda($total_associado*$cha_taxa_percentual));?> -->
             </td>
           </tr>    
			<tr>
             ...
             <th>
             	...
             	$tot_final = $ped_usr_associado==1 ? $total_associado + ($cha_taxa_percentual) : $total_nao_associado + $cha_taxa_percentual;

             	echo(formata_moeda($tot_final));
             	?>
...
<?php
                    ...
                    if($res)
                    {
                       ...
                       <table class="table table-pedido table-striped table-bordered table-condensed table-hover">
						<thead>		 
                            <tr>
                              ...
                              <th>Unidade</th>
                              <th>Preço (R$)</th>
                              <th class="hidden">Preço para<br>Não-Associado (R$)</th>
                       ...
                       while ($row = mysqli_fetch_array($res,MYSQLI_ASSOC)) 
                       {
                            ...
                            <td><?php echo($row["prod_unidade"]);?></td>
							<td><?php echo(formata_numero_de_mysql($row["prod_valor_venda"])); ?></td>
							<td class="hidden"><?php echo(formata_numero_de_mysql($row["prod_valor_venda_margem"])); ?></td> 
							<td>
			...
			<tr>
          	 <td colspan="6"><div align="right">
          	  Taxa de Frete: R$ <?php echo(formata_moeda($cha_taxa_percentual)); ?> 
          	 </div></td> 
             <!-- <?php echo(formata_moeda($total_associado*$cha_taxa_percentual));?> -->
           </tr>
                            ...
                            <tr>
                             <th colspan="5"><div align="right">TOTAL: R$ <?php echo(formata_moeda($total_pedido + $cha_taxa_percentual)  ); ?>  </div>
                             </th>
                             <th class="hidden" id="total_pedido"></th>
                           </tr>
                       </tbody>
					</table>
	...
	if($pedido_enviado)
 	{
		...
		$sql = "SELECT nuc_nome_completo, nuc_entrega_endereco, nuc_entrega_horario, nuc_email FROM nucleos WHERE nuc_id=". prep_para_bd($nuc_id) ;
 		...
 		$res = executa_sql($sql);
  	    if ($row = mysqli_fetch_array($res,MYSQLI_ASSOC)) 
		{		  
			$msg_confirmacao.="Cliente: " . $nome_usuario . "\n";
			$msg_confirmacao.="Área de Entrega: " . $row["nuc_nome_completo"] . "\n";
			$msg_confirmacao.="Endereço: " . $end_usuario . "\n";			
			$msg_confirmacao.="Horário: " . $row["nuc_entrega_horario"] . "\n";
			$msg_confirmacao.="Valor: R$ " . formata_moeda($tot_final) . "\n\n";
//			$msg_confirmacao.="Email: " . $row["nuc_email"] . "\n\n";					
		}
		$msg_confirmacao.=get_texto_interno("txt_email_final_confirmacao");
		
		
		envia_email_cestante($ped_usr,'Pedido Mercearia Organica ',"",$msg_confirmacao);
	}
